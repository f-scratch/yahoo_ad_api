...
RSpec.describe 'ReportService' do
  include_context 'stub.helper.v6.3.success'
  include_context 'config.helper.v6.3'
  include_context 'report.helper.v6.3'
  ...
  describe '#get' do
    ...
    context 'when status is WAIT' do
      include_context 'stub.helper.v6.3.error'
      ...
      it 'cannot get report_download_url' do
        target = subject[:values][0]
        expect(target[:report_record][:report_job_status]).to eq 'WAIT'
      end
    end
  end
end
